...
python src/pegrna_pipeline/01_normalize_variants.py \
...
python src/pegrna_pipeline/02_build_contexts_and_designability.py \
...
python src/pegrna_pipeline/03_select_mutation_panel.py \
    --input $OUTDIR/variants_designability.parquet \
    --config config/pipeline_config.yaml \
    --output-dir $OUTDIR

echo "Phase 4: Design pegRNAs"
python src/pegrna_pipeline/04_design_pegrnas.py \
    --input $OUTDIR/mutation_panel_primary.parquet \
    --config config/pipeline_config.yaml \
    --output-dir $OUTDIR

echo "Phase 5: Validate pegRNA Designs"
python src/pegrna_pipeline/05_validate_pegrna_designs.py \
